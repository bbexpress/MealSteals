...
.factory('Auth', function($cordovaInAppBrowser, $q, $rootScope, $state, $firebaseObject, CordovaCamera, $firebaseArray, Firebase, Timestamp, $cordovaNativeStorage) {
...
//
   $rootScope.checkIn = function(deal){
       ...
       $rootScope.mealsteals.child('/deals/' + deal.key + '/checkIn/' + $rootScope.authUserData.uid).set({'username': $rootScope.currentUser.username, 'timestamp': deal.checkInTime});
   }
   ...
   $rootScope.redeemed = function(deal){
       ...
       deal['checkInTime'] = new Date().getTime();
            ...
            $rootScope.mealsteals.child( '/deals/' + deal.key + '/analytics/redeemAnalytics/' + $rootScope.authUserData.uid).set({'username' : $rootScope.currentUser.username, 'timestamp': deal.checkInTime});
    ...
    $rootScope.newRating = function(deal, type, total, comment){

        if(type == 'recurring'){

            //set in recurring
            $rootScope.mealsteals.child('/recurringDeals/' + deal.recurringDealID + '/ratings/' + $rootScope.authUserData.uid).set($rootScope.recurringObj[deal.recurringDealID].ratings[$rootScope.authUserData.uid]);

            //set in business
            $rootScope.mealsteals.child('/businesses/' + deal.businessID + '/ratings/recurring/' + deal.recurringDealID  + '/' + $rootScope.authUserData.uid).set($rootScope.recurringObj[deal.recurringDealID].ratings[$rootScope.authUserData.uid]);

            $rootScope.mealsteals.child('/deals/' + deal.key + '/totalRating/').set(total);

        }else{
            //set one time deal
            $rootScope.mealsteals.child('/deals/' + deal.key + '/ratings/' + $rootScope.authUserData.uid).set(deal.ratings[$rootScope.authUserData.uid]);
            $rootScope.mealsteals.child('/deals/' + deal.key + '/totalRating/').set(total);
            //set in business
    ...
    function AuthDataCallback(AuthData) {
        if (AuthData) {
            ...
            //
                ...
                if ($rootScope.authUserData) {
                  console.log("Authenticated user with uid:", $rootScope.authUserData.uid);

                }


                    if($rootScope.authUserData.uid == '513bad1b-55f6-4377-92fa-9a7d9656be6e'  ||
                       $rootScope.authUserData.uid == 'c615ea89-b0b6-4249-b8fa-46d445a2e5ec'  ||
                       $rootScope.authUserData.uid == 'f5b9a5e5-9422-45f2-9ad7-9c69ab21c88b'  ||
                       $rootScope.authUserData.uid == 'c2c7b05c-7a8a-402e-b9dc-b8fcfee5c9d6'  ||
                       $rootScope.authUserData.uid == '1e55080c-ae03-45d7-a292-2fca452c05e1'){

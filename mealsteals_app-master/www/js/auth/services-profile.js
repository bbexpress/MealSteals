...
*/
.factory('Profile', function($q, FireFunc, Utils, Codes, CordovaCamera, $rootScope) {
  ...
  self.getGlobal = function(uid, globalProperty) {
    var childRef = "users_app/" + uid + "/" + globalProperty;
    return FireFunc.onValue(childRef).then(
      function(ProfileData){
        // write to cache
        self.ProfileData = ProfileData;
  ...
  self.getSub = function(uid, globalProperty, subProperty) {
    var childRef = "users_app/" + uid + "/" + globalProperty + "/" + subProperty;
    ...
    return CordovaCamera.newImage(sourceTypeIndex, 200).then(
      function(imageData){
        if(imageData != undefined) {
            //$rootScope.uploadFiles(imageData, 'profileImage');
  ...
  self.changeUserName = function(uid, newUsername) {
    ...
    function proceed1() {
      ...
      var oldMetaRef = "users_app/" + uid + "/meta/username"

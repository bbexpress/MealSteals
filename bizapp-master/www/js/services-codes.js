...
.factory('Codes', function(Utils, $q) {
  ...
  self.handleError = function(error) {
    Utils.showMessage(self.getErrorMessage(error), 2500);
  };
  ...
  self.getErrorMessage = function(error) {
    ...
    if (error.hasOwnProperty('code')){
      switch(error.code) {
        case 'INVALID_USER':
          //
          updateMessage = "User does not exist... try again!"
        ...
        case 'EMAIL_TAKEN':
          //
          updateMessage = "You already have an email associated with MealSteals"
          break
        case 'USERNAME_TAKEN':
          //
